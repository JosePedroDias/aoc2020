...
toward the airport. While travel by toboggan might be easy,
...
exact integer coordinates in a grid.
...
start by counting all the trees you would encounter for
...
is `right 3` and `down 1` from there, and so on until you go
...
2, 7, 3, 4, and 2 tree(s) respectively;
